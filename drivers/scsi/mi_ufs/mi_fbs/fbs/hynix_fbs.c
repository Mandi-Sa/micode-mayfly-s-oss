...
#define FBS_HYNIX_LRANGE_HEADER_SIZE 8
#define FBS_HYNIX_LRANGE_ENTRY_SIZE 8

int hynix_get_fbs_support(struct ufsfbs_dev *fbs, u8 *fbs_support_en)
{
...
int hynix_fbs_rec_lba_range_size(struct ufsfbs_dev *fbs, u32 *rec_lrs)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_max_lba_range_size(struct ufsfbs_dev *fbs, u32 *max_lrs)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_min_lba_range_size(struct ufsfbs_dev *fbs, u32 *min_lrs)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_max_lba_range_count(struct ufsfbs_dev *fbs, int *lrc)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_lba_range_alignment(struct ufsfbs_dev *fbs, int *lra)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_level_to_exe_set(struct ufsfbs_dev *fbs, int *level_exe_set)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_level_to_exe_get(struct ufsfbs_dev *fbs, int *level_exe_get)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_level_ops(struct ufsfbs_dev *fbs, int *level_ops)
{
	struct ufs_hba *hba = fbs->hba;
	int ret = 0, attr = -1;
	...
	if (attr == 0x0) {
		...
		*level_ops = FBS_OPS_INTERNAL_ERR;
		...
		FBS_INFO_MSG("[ufsfbs]Hynix unknown level ops %d\n", attr);
		ret = -1;
		return ret;
	}
	return 0;
...
int hynix_defrag_ops(struct ufsfbs_dev *fbs, int *defrag_ops)
{
	struct ufs_hba *hba = fbs->hba;
	int ret = 0, attr = -1;
	...
	if (attr == 0x0) {
		...
		*defrag_ops = FBS_OPS_INTERNAL_ERR;
		...
		FBS_INFO_MSG("[ufsfbs]Hynix unknown defrag ops %d\n", attr);
		ret = -1;
		return ret;
	}
	return 0;
...
int hynix_fbs_frag_level_check_enable(struct ufsfbs_dev *fbs, bool level_check_enable)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_defrag_enable(struct ufsfbs_dev *fbs, bool defrag_enable)
{
	struct ufs_hba *hba = fbs->hba;
...
int hynix_fbs_lba_range_list_write(struct scsi_device *sdev, void *buf, uint8_t mode, uint8_t buf_id, int offset, int len)
{
	struct ufs_hba *hba = shost_priv(sdev->host);
	unsigned char cdb[10] = {0};
	struct scsi_sense_hdr sshdr = {};
	unsigned long flags = 0;
	int ret = 0, len_index = 1, lba_index = FBS_HYNIX_LRANGE_HEADER_SIZE;
	...
	while((lba = strsep(&raw_buf, ",")) != NULL) {
		...
		if (len_index % 2) {
			...
			lba_index += FBS_HYNIX_LRANGE_ENTRY_SIZE;
		}
		len_index++;
	}
	/*create lba range buf send for device*/
	...
	ret = scsi_execute(sdev, cdb, DMA_TO_DEVICE, lba_buf, (FBS_HYNIX_LRANGE_HEADER_SIZE + len*FBS_HYNIX_LRANGE_ENTRY_SIZE), NULL, &sshdr, msecs_to_jiffies(15000), 0, 0, RQF_PM, NULL);
...
int hynix_fbs_read_frag_level(struct scsi_device *sdev, void *buf, uint8_t mode, uint8_t buf_id, int offset, int len)
{
	uint8_t cdb[16];
	int ret = 0;
	...
	ret = scsi_execute(sdev, cdb, DMA_FROM_DEVICE, buf, (FBS_HYNIX_LRANGE_HEADER_SIZE + len*FBS_HYNIX_LRANGE_ENTRY_SIZE), NULL, &sshdr, msecs_to_jiffies(15000), 0, 0, RQF_PM, NULL);
	if (ret)
...
struct ufsfbs_ops hynix_fbs_ops = {
		.get_fbs_support = hynix_get_fbs_support,
		.fbs_rec_lba_range_size = hynix_fbs_rec_lba_range_size,
		.fbs_max_lba_range_size = hynix_fbs_max_lba_range_size,
		.fbs_min_lba_range_size = hynix_fbs_min_lba_range_size,
		.fbs_max_lba_range_count = hynix_fbs_max_lba_range_count,
		.fbs_lba_range_alignment = hynix_fbs_lba_range_alignment,
		.fbs_level_to_exe_set = hynix_fbs_level_to_exe_set,
		.fbs_level_to_exe_get = hynix_fbs_level_to_exe_get,
		.fbs_level_ops = hynix_fbs_level_ops,
		.fbs_defrag_ops = hynix_defrag_ops,
		.fbs_frag_level_check_enable = hynix_fbs_frag_level_check_enable,
		.fbs_defrag_enable = hynix_fbs_defrag_enable,
		.fbs_lba_range_list_write = hynix_fbs_lba_range_list_write,
		.fbs_read_frag_level = hynix_fbs_read_frag_level
};
