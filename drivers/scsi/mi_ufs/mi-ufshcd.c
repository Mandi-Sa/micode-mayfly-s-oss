...
#define QUERY_REQ_TIMEOUT 5500 /* 5.5 seconds original 1.5 */
...
static struct ufs_dev_fix ufs_fixups[] = {
	...
	UFS_FIX(UFS_VENDOR_TOSHIBA, "THGLF2G9D8KBADG",
		UFS_DEVICE_QUIRK_PA_TACTIVATE),
	UFS_FIX(UFS_VENDOR_SAMSUNG, "KLUDG4UHGC-B0E1",
		UFS_DEVICE_QUIRK_PA_SYNCLENGTH),
	UFS_FIX(UFS_VENDOR_SAMSUNG, "KLUEG8UHGC-B0E1",
		UFS_DEVICE_QUIRK_PA_SYNCLENGTH),
	UFS_FIX(UFS_VENDOR_SAMSUNG, "KLUFG8RHGB-B0E1",
		UFS_DEVICE_QUIRK_PA_SYNCLENGTH),
	UFS_FIX(UFS_VENDOR_WDC, "SDINFDO4-256G",
		UFS_DEVICE_QUIRK_PA_SYNCLENGTH),
	UFS_FIX(UFS_VENDOR_WDC, "SDINFDO4-512G",
		UFS_DEVICE_QUIRK_PA_SYNCLENGTH),
	END_FIX
};
...
ufshcd_dispatch_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	ufshcd_writel(hba, uic_cmd->command & COMMAND_OPCODE_MASK,
		      REG_UIC_COMMAND);
	wmb();
	temp = ufshcd_readl(hba, REG_UIC_COMMAND);
	if ((temp & COMMAND_OPCODE_MASK) != (uic_cmd->command & COMMAND_OPCODE_MASK)){
			dev_err(hba->dev, "UIC CMD readback mismatching! W: 0x%x / R:0x%x\n",uic_cmd->command, temp);
			WARN_ON(1);
	}
}
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	/*recongnize fbs cmd*/
	scsi_cmd_mode = cmd->cmnd[1] & 0x1f;

	if(scsi_cmd_mode == fbs_mode){
		lrbp->lun = 0;
	}
...
out:
	if  (ret) {
		ufsdbg_set_err_state("uic_pwr_ctrl fail\n");
...
static void ufshcd_tune_unipro_params(struct ufs_hba *hba)
{
	...
	/*set synclength for samsung && sandisk*/
	if(pa_hibern8time_quirk_enabled){
		ufshcd_dme_peer_set(hba, UIC_ARG_MIB(0x15D0), 0x4F);  // Synclength G4
		ufshcd_dme_peer_set(hba, UIC_ARG_MIB(0x1552), 0x4F);  // Synclength G1
	}
}
